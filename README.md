...
* **Interactive Viewport Border Drawing:** Features an active live structural render bounds bounding-box view overlay inside your viewport utilizing dedicated Blender GPU batch shaders.
* **State Preservation Architecture:** Automatically backs up native raw resolution when toggled and restores baseline resolution fidelity cleanly.

---

## 🛠️ Installation

1.  **Download** the latest version source bundle.
...
* **Reset Frame** : Destroys all localized factor alterations, turns off border parsing buffers, and sets dimensions back to initial values.
* **Match Active Scene Dimensions** : Pulls standard dimensions directly into your layout settings.
* **Multiplier Scale Engine** : Multiplies output metrics safely up to `8×` baseline metrics for extreme resolution renders.

### 📐 Automated Composition Framing

* **Auto Crop**: Evaluates coordinates for selected meshes in your scene. It adds safety margins, locks orientation aspect variables, and sets up optimal framing based on what the active camera sees.
* **Template Injections**: Instantly structures composition frameworks using the following layouts:
